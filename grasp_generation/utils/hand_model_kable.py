...
class HandModel:
            ...
            if(len(body.link.visuals) > 0):
                ...
                for visual in body.link.visuals:
                    ...
                    if visual.geom_type == "box":
                        ...
                        # print(dir(visual.offset))
                        # print(visual.offset.pos)
                        rotation_matrix = tm.transformations.rotation_matrix(
                            angle=np.radians(90),  # 角度转为弧度
                            direction=[0, 1, 0],   # 绕 Y 轴旋转
                            # point=[0, 0, 0]        # 旋转中心为原点
                        )
                        link_mesh = tm.primitives.Capsule(radius=visual.geom_param[0], height=visual.geom_param[1] * 2).apply_transform(rotation_matrix)
                ...
                # # generate condact points
                # link_vertices_numpy = link_vertices.numpy()
                # selected_points = self.select_points(link_vertices_numpy, 14, cluster_axes='xz', max_axis='y')
                # # print(selected_points)
                # formatted = ",\n".join("[" + ", ".join(f"{value:.8f}" for value in row) + "]" for row in selected_points)
                # print(formatted)
                # # quit()
    ...
    def select_points(self, points, num_points, cluster_axes='xy', max_axis='z'):
        """
        Selects `num_points` from the 3D points based on the highest values
        in the specified `max_axis` while ensuring coverage across the specified
        `cluster_axes`.

        Args:
            points (numpy.ndarray): Array of shape (N, 3) representing 3D points.
            num_points (int): The number of points to select.
            cluster_axes (str): Axes for clustering, e.g., 'xy', 'xz', 'yz'.
            max_axis (str): Axis to maximize, e.g., 'x', 'y', 'z'.

        Returns:
            list: A 2D list of selected points.
        """
        # Define axis indices
        axis_map = {'x': 0, 'y': 1, 'z': 2}
        cluster_indices = [axis_map[axis] for axis in cluster_axes]
        max_index = axis_map[max_axis]

        # Step 1: Cluster the points based on the specified axes
        kmeans = KMeans(n_clusters=num_points)
        kmeans.fit(points[:, cluster_indices])  # Use specified axes for clustering

        # Step 2: Select the point with the highest value in the specified max_axis from each cluster
        selected_points = []
        for cluster_label in range(num_points):
            cluster_points = points[kmeans.labels_ == cluster_label]
            if len(cluster_points) > 0:
                # Select the point with the maximum value in the specified max_axis
                max_point = cluster_points[np.argmax(cluster_points[:, max_index])]
                selected_points.append(max_point)

        return np.array(selected_points).tolist()
    ...
    def cal_distance(self, x):
        ...
        for link_name in self.mesh:
            if link_name in ['base_link', 'Thumb_Side_Gear_Link_child']:
                continue
            # print(link_name)
